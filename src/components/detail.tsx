      ...
      <DialogContent className="detail-dialog">
        <div className="detail-preview">
          ...
          <div className="preview-caption">
            {media ? (
              <>
                <span>{media.raw_name}</span>
                <a
                  href={`https://drive.google.com/file/d/${media.drive_id}/view`}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="Open original in Google Drive"
                >
                  <ArrowUpRight size={16} />
                </a>
              </>
            ) : (
              <span>{members.length} items in this set</span>
            )}
          </div>
        </div>
        <div className="detail-panel">
          <DialogHeader>
            <div className="eyebrow">
              {selected.kind === "media" ? "MEDIA DETAILS" : "SET DETAILS"}
            </div>
            <DialogTitle>{item.display_name}</DialogTitle>
            <DialogDescription>
              {media
                ? "Make this file easy to find."
                : "A little context for your collection."}
            </DialogDescription>
          </DialogHeader>
          <form
            onSubmit={(e) => {
          ...
          {selected.kind === "set" && (
            <div className="delete-set">
              {confirmDelete ? (
                <>
                  <p>Delete this set? Its media will stay in your library.</p>
                  <Button
                    variant="destructive"
