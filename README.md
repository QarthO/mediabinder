...
A private media library for Google Drive. TanStack Start, React Query and Table, shadcn/Radix, Better Auth, and MySQL. A compact blue workspace inspired by Kiln, with grid/list views, search, tags, sets, and ⌘K.
...
4. Open **http://localhost:3100**. Sign in with Google, choose a source folder in Settings, save it, and Sync Drive.
...
The app requests `drive.readonly`, plus Google's standard sign-in scopes. Google grants this permission across the account; the selected folder is an application-level import boundary, not an OAuth permission boundary. Folder selection works with shared folders accessible to your account and scans subfolders. Shortcuts are not followed. No Drive write permissions are requested.
...
- Sync is manual. It paginates Drive results and walks the source's subfolders, then reconciles in a transaction. Concurrent syncs for the same account are prevented with a MySQL lock.
- Re-sync updates technical file metadata without overwriting your display names, tags, creation dates, sets, or post links. Files removed from the source are marked unavailable; their metadata remains intact. A failed Drive scan does not mark files unavailable.
...
Use T3 Code's browser preview at http://localhost:3100 to check the real Google login and Drive flows. No development authentication bypass is included. `/api/health` checks the database connection. Back up MySQL before upgrading; the first schema is in `migrations/001-app.sql`, and Better Auth manages its own tables during startup.
